 ...
 *
 */
public class Exercise_13_2 extends JPanel {

  /**
   * The main routine simply opens a window that shows a Exercise_13_2 panel.
   */
  public static void main(String[] args) {
    JFrame window = new JFrame("Simple Paint");
    Exercise_13_2 content = new Exercise_13_2();
  ...
  private enum Tool { CURVE, LINE, RECT, OVAL, FILLED_RECT, FILLED_OVAL, 
    STROKED_RECT, STROKED_OVAL, SMUDGE, ERASE }
   ...
   */
  private final static EnumSet<Tool> SHAPE_TOOLS = EnumSet.range(Tool.LINE, Tool.STROKED_OVAL);
  ...
  private void putCurrentShape(Graphics g) {  
    ...
    case LINE:
      ...
      break;
    case OVAL:
      putOval(g, false, false, startX, startY, currentX, currentY);
      break;
    case RECT:
      putRect(g, false, false, startX, startY, currentX, currentY);
      break;
    case FILLED_OVAL:
   ...
   */
  private void putRect(Graphics g, boolean filled, boolean stroked, int x1, int y1, int x2, int y2) {  
    if (x1 == x2 || y1 == y2)
      return;
   ...
   */
  private void putOval(Graphics g, boolean filled, boolean stroked, int x1, int y1, int x2, int y2) {
    if (x1 == x2 || y1 == y2)
      return;
    if (x2 < x1) { // Swap x1,x2 if necessary to make x2 > x1.
      int temp = x1;
  ...
  public JMenuBar getMenuBar() {
    ...
    JMenu colorMenu = new JMenu("Stroke Color");
    JMenu strokeFillMenu = new JMenu("Stroke Fill");
    ...
    toolMenu.add(createJMenuItem("Stroked Filled Rectangle", listener));
    toolMenu.add(createJMenuItem("Stroked Filled Oval", listener));
    toolMenu.addSeparator();
    ...
    strokeWidthMenu.add(createJMenuItem("1px", listener));
    strokeWidthMenu.add(createJMenuItem("2px", listener));
    strokeWidthMenu.add(createJMenuItem("4px", listener));
    strokeWidthMenu.add(createJMenuItem("8px", listener));
    strokeFillMenu.add(createJMenuItem("Fill With Black", listener));
    strokeFillMenu.add(createJMenuItem("Fill With White", listener));
    strokeFillMenu.add(createJMenuItem("Fill With Red", listener));
    strokeFillMenu.add(createJMenuItem("Fill With Green", listener));
    strokeFillMenu.add(createJMenuItem("Fill With Blue", listener));
    strokeFillMenu.add(createJMenuItem("Fill With Yellow", listener));
    return menubar;
  }
  ...
  private class MenuHandler implements ActionListener {
    public void actionPerformed(ActionEvent evt) {
      ...
      case "Select Drawing Color...":
        newColor = JColorChooser.showDialog(Exercise_13_2.this, "Select Drawing Color", currentColor);
        if (newColor != null)
          currentColor = newColor;
        break;
      case "Fill With Color...":
        newColor = JColorChooser.showDialog(Exercise_13_2.this, "Select Fill Color", bgColor);
        if (newColor != null) {
          bgColor = newColor;
          Graphics osg = OSC.createGraphics();
          osg.setColor(bgColor);
      ...
      case "Filled Oval":
        ...
        break;
      case "Stroked Filled Oval":
        System.out.println("Stroked oval");
        currentTool = Tool.STROKED_OVAL;
        break;
      case "Stroked Filled Rectangle":
        currentTool = Tool.STROKED_RECT;
        break;
      case "Smudge":
      ...
      case "Undo":
        undo();
        break;
      case "1px": case "2px": case "4px": case "8px":
        stroke = new BasicStroke(Float.parseFloat(evt.getActionCommand().substring(0, 1)));
        break;
      case "Fill With Black":
        fillColor = Color.BLACK;
        break;
      case "Fill With White":
        fillColor = Color.WHITE;
        break;
      case "Fill With Red":
        fillColor = Color.RED;
        break;
      case "Fill With Green":
        fillColor = Color.GREEN;
        break;
      case "Fill With Blue":
        fillColor = Color.BLUE;
        break;
      case "Fill With Yellow":
    ...
    private void saveFile(Image OSC, String format) {
        ...
        dialog.showSaveDialog(Exercise_13_2.this);
        File file = dialog.getSelectedFile();
        
        int choice = -1;
        if (file.exists()) {
          choice = JOptionPane.showConfirmDialog(Exercise_13_2.this, "Are you sure you want to overwrite the existing "
          + file.getName() + " file?");
      ...
      private void openFile() {
        JFileChooser dialog = new JFileChooser();
        int option = dialog.showOpenDialog(Exercise_13_2.this);
        if (option == JFileChooser.APPROVE_OPTION)
        try {
          BufferedImage img = ImageIO.read(dialog.getSelectedFile());
          OSC = img;
          repaint();
        } catch (IOException e) {
          e.printStackTrace();
  ...
  private class MouseHandler implements MouseListener, MouseMotionListener {
    ...
    void applyToolAlongLine(int x1, int y1, int x2, int y2) {      
      ...
      g.setColor(bgColor); // (for ERASE only)
